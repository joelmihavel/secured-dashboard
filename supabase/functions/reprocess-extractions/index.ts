...
const EXTRACTION_RESPONSE_SCHEMA = {
  ...
  properties: {
    ...
    rent_due_day: { type: "integer", description: "Day of month (1-28) representing the LAST day rent can be paid without penalty / late fee — i.e., the grace-inclusive payment deadline. If the agreement says 'rent due on 1st' with no grace, return 1. If it says 'rent due on 1st with grace period of 4 days', return 5 (1+4=5). If it says 'rent payable within 5th day in advance', return 5. If it says 'penalty 1% per day after the 5th', return 5. If it says 'rent payable from 1st to 5th of every month', return 5. ALWAYS return the END date of any grace window — never the original due day when grace is mentioned. The field rent_grace_period_days is deprecated and should always be null in your response (grace is folded into rent_due_day).", nullable: true },
    rent_grace_period_days: { type: "integer", description: "DEPRECATED in Phase 2 (single-field semantic). Always return null. Grace days are now folded into rent_due_day directly — see the rent_due_day description for the math. This field is kept in the schema only so legacy extractions in the database remain readable; AI must NOT populate it.", nullable: true },
