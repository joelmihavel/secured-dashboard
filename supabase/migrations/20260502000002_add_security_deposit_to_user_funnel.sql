-- Extend v_user_funnel with security_deposit_paise so the admin Google Sheet
-- can show the actual deposit instead of the rent value (the Apps Script was
-- using rent as a placeholder because the view never exposed the column).
--
-- security_deposit_paise already lives on extracted_rental_info — only the
-- view's outer SELECT list needs it. The LATERAL subquery already pulls
-- ei.* implicitly (via SELECT *), so no JOIN changes are required.
--
-- Pure additive change:
--   - No columns removed or renamed
--   - All existing joins/logic preserved verbatim from 20260424000004
--   - One new column pulled from extracted_rental_info
--
-- Postgres can't ALTER VIEW ADD COLUMN, so the whole view is dropped and
-- recreated. Existing readers use .select("*") so new columns propagate
-- automatically.
...
-- Pattern matches the prior 20260424000004 grants exactly.
...
-- rollback:
--   DROP VIEW IF EXISTS public.v_user_funnel;
--   -- Then re-run 20260424000004 to recreate the previous-shape view (without
--   -- the new security_deposit_paise column).
