-- Fix: v_user_funnel extraction join should prefer the extraction linked
-- via waitlist_entries.extraction_id when available. This ensures waitlisted
-- users always show their agreement data (the extraction they had when joining).
...
-- Extraction: prefer the one linked via waitlist_entries.extraction_id, fall back to latest completed
LEFT JOIN LATERAL (
