-- Surface the landlord phone collected during the invite flow.
--
-- v_user_funnel.landlord_phone was previously sourced only from
-- extracted_rental_info.landlord_phone, which Gemini barely ever extracts
-- from rental agreements (1 of 605 rows populated). The phone we collect
-- when the tenant invites the landlord lives on tenancies.landlord_phone
-- (41 of 386 rows populated), and that's the value the admin Landlords
-- sheet is meant to display.
--
-- Fix: pull landlord_phone into the tenancy lateral subquery and
-- COALESCE the outer projection so the column shows the invite-flow phone
-- when present, falling back to whatever Gemini extracted as a last resort.
--
-- Pure additive change for downstream readers — column name unchanged,
-- still nullable. All existing readers use SELECT *.
--
-- Postgres can't ALTER VIEW ADD COLUMN, so the whole view is dropped and
-- recreated. Verbatim from 20260502000002 except the two lines noted.

-- @safe-destructive: view is recreated immediately below with the same
-- column shape (only the *value* of landlord_phone changes for rows where
-- the tenancy has a phone but the extraction doesn't — strictly an
-- improvement). No data loss possible (views hold no data).
...
SELECT
  ...
  -- Prefer the phone collected during invite over whatever Gemini extracted
...
-- rollback:
--   DROP VIEW IF EXISTS public.v_user_funnel;
--   -- Then re-run 20260502000002 to restore the previous shape (landlord_phone
--   -- sourced only from extracted_rental_info).
