#!/usr/bin/env node
/**
 * generate-prd.js — Decomposes the 42-story prd.json into ~130 Ralph micro-stories.
 *
 * Reads: autobot/prd.json (current backlog), buildbot/config/screen-routes.json
 * Writes: autobot/prd.json (new decomposed format)
 *
 * Usage: node autobot/generate-prd.js [--dry-run]
 */
...
const inProgressOldIds = new Set();

function collectOldStoryStatuses(workStream) {
...
const uiStories = oldPrd.workStreams?.ui?.stories || [];
...
for (const ui of uiStories) {
  ...
  stories.push({
    ...
    dependsOn: [extractId],
    acceptanceCriteria: [
      `Screen file exists at ${ui.appFile || (ui.appFiles && ui.appFiles[0]) || "rn-app/app/..."}`,
...
const backendStories = oldPrd.workStreams?.backend?.stories || [];

for (const be of backendStories) {
  const isCompleted = completedOldIds.has(be.id);
  const fnName = (be.existingFunctions && be.existingFunctions[0]) || be.id.toLowerCase();
...
const stateStories = oldPrd.workStreams?.state?.stories || [];
...
const prodStories = oldPrd.workStreams?.production?.stories || [];
...
const testStories = oldPrd.workStreams?.testing?.stories || [];
...
const newPrd = {
  ...
  meta: {
    ...
    failed: 0,
    lastIteration: 0,
    generatedFrom: "autobot/generate-prd.js",
  },
};
