 ...
 *  Agreement routes (/intro, /upload, /setup-intro, /add-bank-details) are
 *  always re-resolved from user_status — they're transient by nature. */
 ...
 *      'approved' → re-route to /setup-intro.
 *    - /(setup)/add-bank: deferred PAN matching can flip bank_verified in
 *      background; status can advance approved → active.
 *
 *  Those routes still live in VALID_CACHED_ROUTES (so the cache is written
 *  + used as offline fallback), they just don't fast-path-display. The
 *  trade-off: stable waitlisted / mid-setup users see a 1-3s splash on
 *  cold start instead of an instant cache hit. Worth it to never flash the
 *  wrong screen on a state-flip. */
...
export default function Index() {
  ...
  const resolveAuthenticatedJourney = useCallback(async (userId: string) => {
    ...
    try {
      ...
      // just /(main)). Transient routes (/(waitlist), /(setup)/add-bank) skip
      // the fast-path and wait for background validation, otherwise they
      // flash the wrong screen when user_status flipped while app was closed
      // (e.g. waitlisted → approved by admin). Avoids 1-3s of network calls
