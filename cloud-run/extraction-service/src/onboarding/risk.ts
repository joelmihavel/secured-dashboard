/**
 * Flent Secured v2 - Risk Assessment (Cloud Run Port)
 *
 * Ported from: supabase/functions/_shared/risk-utils.ts
 *
 * Computes a multi-signal risk score for waitlist entries.
 * 14 weighted signals across two phases (pre + post waitlist).
 *
 * PRE-WAITLIST (phase: "pre"):
 *  1. pan_verification       (wt 3)
 *  2. bank_verification      (wt 3)
 *  3. name_consistency       (wt 3)
 *  4. agreement_completeness (wt 2)
 *  5. agreement_expiry       (wt 4)
 *  6. phone_duplicate        (wt 5)
 *  7. rent_reasonableness    (wt 2)
 *  8. tenant_name_match      (wt 4)
 *
 * POST-WAITLIST (phase: "post") -- SKIPPED if no data:
 *  9. m360_risk_intel        (wt 4)
 * 10. m360_data_available    (wt 3)
 * 11. credit_score           (wt 3)
 * 12. utility_verification   (wt 2)
 * 13. landlord_response      (wt 3)
 * 14. agreement_confidence   (wt 2)
 */
...
function normalizeNameForComparison(name: string): string {
  return name
    ...
    .replace(/\(HUF\)/gi, "")
    .replace(
      /\b(MR|MRS|MS|DR|SHRI|SMT|KUMARI|LATE|PROF|S\/O|W\/O|D\/O)\b/gi,
      ""
    )
    .replace(/[^A-Z\s]/g, "")
    .replace(/\s+/g, " ")
  ...
  try {
    ...
    const [
      ...
      supabase
        ...
        .select(
          "name_verified, address_verified, name_match_score, address_match_score, status"
        )
        .eq("user_id", userId)
        .order("created_at", { ascending: false })
        .limit(1)
    ...
    const user = userResult.data as Record<string, any> | null;
    const iv = ivResult.data as Record<string, any> | null;
    const extraction = extractionResult.data as Record<string, any> | null;
    const bank = bankResult.data as Record<string, any> | null;
    const utility = utilityResult.data as Record<string, any> | null;
    const tenancy = tenancyResult.data as Record<string, any> | null;
    const _extractionCount = extractionCountResult.count ?? 0;
    ...
    if (bank) {
      ...
      const agreementNameMatched = bank.agreement_name_matched as
        | boolean
        | null;
      ...
      if (bankVerified === true && agreementNameMatched === true) {
        ...
        bankSignal = "RED";
        bankDetail =
          "Penny drop failed -- bank account could not be verified";
      } else if (
        bankVerified === true &&
        agreementNameMatched === false
      ) {
        bankSignal = "YELLOW";
        ...
        bankSignal = "YELLOW";
        bankDetail =
          "Penny drop succeeded but account not marked verified";
      } else {
        bankSignal = "YELLOW";
        bankDetail = `Bank verification pending (status=${pennyDropStatus ?? "none"})`;
      }
      factors.push({
        factor: "bank_verification",
        signal: bankSignal,
        weight: 3,
        detail: bankDetail,
        phase: "pre",
    ...
    if (bank) {
      ...
      const landlordNames =
        (extraction?.landlord_names as string[] | null) ?? [];
      ...
      if (landlordNames.length > 0)
        sources.push({ label: "Agreement", name: landlordNames[0] });
      ...
      if (sources.length >= 2) {
        ...
        for (let i = 0; i < sources.length; i++) {
          for (let j = i + 1; j < sources.length; j++) {
            totalPairs++;
            if (namesAreSimilar(sources[i].name, sources[j].name))
              matchCount++;
          }
        }
    ...
    // Signal 4: agreement_completeness (weight 2)
    {
      ...
      if (!extraction) {
        ...
        const needsReview = extraction.needs_manual_review as
          | boolean
          | null;
        const confScore = extraction.confidence_score as number | null;
        const hasAddress = !!extraction.property_address;
        const hasRent =
          (extraction.monthly_rent_paise as number | null) != null &&
          (extraction.monthly_rent_paise as number) > 0;
        const hasNames =
          !!(extraction.landlord_names as string[] | null)?.length &&
          !!(extraction.tenant_names as string[] | null)?.length;
        const hasDates = !!extraction.lease_start_date;
        const keyFieldsPresent =
          hasAddress && hasRent && hasNames && hasDates;
        ...
        if (
          ...
          status === "completed" &&
          ((confScore ?? 0) >= 50 || needsReview || !keyFieldsPresent)
        ) {
          const issues: string[] = [];
          if ((confScore ?? 0) < 80)
            issues.push(`confidence=${confScore}%`);
    ...
    // Signal 5: agreement_expiry (weight 4)
    {
      ...
      if (leaseEndDate) {
        ...
        aeSignal = "YELLOW";
        aeDetail =
          "Lease end date not extracted -- cannot verify agreement validity";
      }
      factors.push({
    ...
    // Signal 7: rent_reasonableness (weight 2)
    {
      ...
      if (rentPaise == null || rentPaise <= 0) {
        ...
        if (rentRupees >= 5000 && rentRupees <= 500000) {
          rrSignal = "GREEN";
          rrDetail = `Rent Rs.${rentRupees.toLocaleString("en-IN")}/month -- within expected range`;
        } else {
          rrSignal = "RED";
          rrDetail = `Rent Rs.${rentRupees.toLocaleString("en-IN")}/month -- outside expected range (Rs.5K-Rs.5L)`;
        }
      }
      factors.push({
        factor: "rent_reasonableness",
        signal: rrSignal,
        weight: 2,
        detail: rrDetail,
        phase: "pre",
    ...
    // Skipped entirely if no data exists.
    // ========================================

    // Signal 9: m360_risk_intel (weight 4)
    if (iv) {
    ...
    // Signal 10: m360_data_available (weight 3)
    if (iv) {
    ...
    // Signal 11: credit_score (weight 3)
    if (iv) {
    ...
    // Signal 13: landlord_response (weight 3)
    if (tenancy) {
      const landlordStatus = tenancy.landlord_status as string | null;
      const landlordApproved = tenancy.landlord_approved as
        | boolean
        | null;
      ...
      // If landlord_status is "none" or null, skip
    }
    ...
    const totalScore = factors.reduce(
      (sum, f) => sum + SIGNAL_SCORE[f.signal] * f.weight,
      0
    );
    ...
    const hasHighWeightRed = factors.some(
      (f) => f.signal === "RED" && f.weight >= 4
    );
 ...
 * Use this instead of calling computeRisk + manual update.
 */
export async function recomputeAndStoreRisk(
  ...
  const hasPostSignals = result.risk_factors.some((f) =>
    postOnlyFactors.includes(f.factor)
  );
