...
function loadExtractionData(figmaId: string): any {
  ...
  const possiblePaths = [
    // 1. AI-enhanced extraction in figma-parity
    path.join(DATA_DIR, 'ai-enhanced', figmaId, 'enhanced-extraction.json'),
    path.join(DATA_DIR, 'ai-enhanced', normalizedId, 'enhanced-extraction.json'),
    // 2. Full extraction from autonomous-parity-fixer (preferred)
    path.join(BASE_DIR, '..', 'autonomous-parity-fixer', 'data', 'combined', normalizedId, 'extraction.json'),
    path.join(BASE_DIR, '..', 'autonomous-parity-fixer', 'data', 'combined', normalizedId, 'local-extraction.json'),
    // 3. Local extraction in figma-parity
    path.join(DATA_DIR, 'screens', normalizedId, 'extracted-values.json'),
    path.join(DATA_DIR, 'screens', figmaId, 'extracted-values.json'),
    // 4. Style map from autonomous-parity-fixer
    path.join(BASE_DIR, '..', 'autonomous-parity-fixer', 'data', 'combined', normalizedId, 'style-map.json'),
    path.join(BASE_DIR, '..', 'autonomous-parity-fixer', 'data', 'combined', figmaId, 'style-map.json'),
    // 5. Constants file from autonomous-parity-fixer
    path.join(BASE_DIR, '..', 'autonomous-parity-fixer', 'data', 'combined', normalizedId, 'constants.json'),
  ];
  ...
  for (const extractionPath of possiblePaths) {
    if (fs.existsSync(extractionPath)) {
      ...
      if (extractionPath.includes('extraction.json') || extractionPath.includes('local-extraction.json')) {
        // Full extraction format from autonomous-parity-fixer - has flat `nodes` array
