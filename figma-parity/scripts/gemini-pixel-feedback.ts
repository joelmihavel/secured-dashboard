/**
 * Gemini 2.5 Pro Pixel-Perfect Feedback System
 *
...
const GEMINI_MODEL = 'gemini-2.5-pro'; // Gemini 2.5 Pro stable release
...
function extractTextNodes(node: any, parentId?: string): FigmaTextNode[] {
  ...
  if (node.nodeType === 'TEXT' || node.figmaData?.type === 'TEXT') {
    const style = node.figmaData?.style || node.style || {};
    const textNode: FigmaTextNode = {
      nodeId: node.nodeId || node.id,
      nodeName: node.nodeName || node.name || '',
      characters: node.figmaData?.characters || node.characters || '',
      textAlignHorizontal: style.textAlignHorizontal || null,
      textAlignVertical: style.textAlignVertical || null,
      fontSize: style.fontSize || node.computedStyles?.fontSize || 0,
      lineHeight: style.lineHeightPx || style.lineHeight || node.computedStyles?.lineHeight || 0,
      fontWeight: style.fontWeight || node.computedStyles?.fontWeight || 400,
      fontFamily: style.fontFamily || node.computedStyles?.fontFamily || '',
      fills: node.figmaData?.fills || node.fills || [],
      characterStyleOverrides: node.figmaData?.characterStyleOverrides || [],
      styleOverrideTable: node.figmaData?.styleOverrideTable || {},
...
function extractPositionedNodes(node: any, parentId?: string): FigmaPositionedNode[] {
  ...
  if (geometry && (geometry.x !== undefined || geometry.width !== undefined)) {
    nodes.push({
      nodeId: node.nodeId || node.id,
      nodeName: node.nodeName || node.name || '',
      nodeType: node.nodeType || node.figmaData?.type || 'UNKNOWN',
      geometry: {
        x: geometry.x || 0,
        y: geometry.y || 0,
...
// Load Figma extraction data
function loadExtractionData(figmaId: string): any {
  const extractionPath = path.join(DATA_DIR, 'ai-enhanced', figmaId, 'enhanced-extraction.json');

  if (!fs.existsSync(extractionPath)) {
    console.error(`Extraction data not found: ${extractionPath}`);
    return null;
  }
...
function findReactNativeCode(route: string): { screenCode: string; componentCodes: Map<string, string> } {
  ...
  // Map route to component directories
...
async function runPixelFeedbackPipeline(screenRoute: string): Promise<FinalReport> {
  ...
  const screenshotsDir = path.join(DATA_DIR, 'ai-enhanced', screen.figmaId, 'screenshots');
